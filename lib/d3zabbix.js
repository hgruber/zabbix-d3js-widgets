...
function timeSeries(arg) {
   ...
   function startRequests() {
    ...
    function plotTimeBands() {
        var incr, firstFull;
        if (sizing[3] == 'm') incr = 60000;
        if (sizing[3] == 'h') incr = 3600000;
        if (sizing[3] == 'd' || sizing[3] == 'M') {
            incr = 86400000;
        ...
        var intervals = [];
        if (sizing[3] != 'M') while (firstFull < xRange[1]) {
            var a = firstFull;
        ...
        var timeBands = bands.selectAll(".bands")
        ...
        var bandsEnter = timeBands.enter()
            ...
            .attr('class', function(d) {
                var a = new Date(d[0]);
                if (sizing[3] == 'h') {
                    if (a.getHours() % 2 == 0) return 'bands hours_even'; else return 'bands hours_odd';
                } else if (sizing[3] == 'm') {
                    if (a.getMinutes() % 2 == 0) return 'bands minutes_even'; else return 'bands minutes_odd';
                } else if (sizing[3] == 'd') {
                    if (a.getDay() == 0 || a.getDay() == 6) return 'bands weekend';
                    else return 'bands workday';
                } else if (sizing[3] == 'M') {
                    if (a.getMonth() % 2 == 0) return 'bands months_even';
                    else return 'bands months_odd';
                }
            });
        timeBands.exit().remove();
        if (sizing[3] != 'd') intervals = [];
