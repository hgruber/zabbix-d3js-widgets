...
function triggerTable(arg) {
    ...
    var params = {
        ...
        'skipDependent': 1,
        'output': [ 'triggerid', 'state', 'error', 'url', 'description', 'comments', 'priority', 'lastchange' ],
        'selectHosts': ['name'],
    ...
    var severity = ['not classified', 'information', 'warning', 'average', 'high', 'disaster' ]; 
    ...
    function row(d) {
        ...
        if (!d.url) {      // so let's take the url from comments' field
            var pattern = /http.*/g;
            d.url = pattern.exec(d.comments);
            d.comments = d.comments.replace(pattern,'').slice(0,-1);
        }
        // TODO: do this in a cleaner way
        return '<table class=triggers title="[' + severity[d.priority] + '] ' + d.comments +
            '"><tr><td class=description><b>' +
            d.hosts[0].name + '</b>: ' +
            ((d.url)?'<a href="'+d.url+'" target=\"_blank\">':'') + d.description +
            ((d.url)?'</a>':'') +
    ...
    function successMethod(response, status) {
        ...
        var p = d3.select(arg.bindTo)
            .selectAll("div")
            .sort(function(a,b) {
                return d3.descending(a.lastchange, b.lastchange);
            })
        ...
        p.enter()
            ...
            // TODO: the acknowledge function has to go to row function
/*            .on('click', function(d) {
...
// displayItem
//
// simply write out a single value
// (or a sum of some values) as a single self updating digit
//
function displayItem(arg) {
    var refresh = typeof arg.refresh !== 'undefined' ? arg.refresh : 10;
    var width = $(arg.bindTo).width();
    start_.push(startRequests);
    function startRequests() {
        req('item.get', {
                'itemids': arg.itemid,
                'output': ['name', 'key_', 'lastvalue', 'lastclock', 'units'],
            },
            itemCallback, errorMethod);
        setTimeout(startRequests, refresh * 1000);
    }
    function errorMethod() {
        console.log('request failed');
    ...
    function itemCallback(response, status) {
        ...
        var units = response.result[0].units;
        $(arg.bindTo).html(String(value) + units);
    }
}
...
// shows an ordered list of items
// that reorder in fixed intervals
//
function orderedItems(arg) {
    var refresh = typeof arg.refresh !== 'undefined' ? arg.refresh : 10;
    start_.push(startRequests);
    var width = $(arg.bindTo).width();
    var items = []
    ...
    function itemCallback(response, status) {
        ...
        function text(d) {
            return '<span>'+d.name+'</span><span style="float: right;">'+d.value+'</span>';
        }
        function top(d) {
            return (d.position*20)+'px';
        }
        now = new Date();
        width = $(arg.bindTo).width();
        last = (now/1000)-600;
        data = [];
        response.result.sort(function(a,b) { return b.lastvalue-a.lastvalue; });
        position = 0;
        response.result.forEach(function(item) {
            if (item.lastclock > last)
                data.push({
                    'name': item.name,
                    'key': item.key_,
...
function timeSeries(arg) {
    ...
    var colors = { 0: 'steelblue', 1: 'red', 2: 'green'};
    ...
    var margin = { top: 16, right: 12, bottom: 12, left: 50 };
    ...
    // interpolation
    // see http://www.d3noob.org/2013/01/smoothing-out-lines-in-d3js.html
    var interpolate = typeof arg.interpolate !== 'undefined' ? arg.interpolate : 'linear';
    ...
    // return uniq colors for a given text label
    function getColor(str) {
        if (colors[str]) {
            ...
            colors[str] = '#' + Math.abs(hash).toString(16).substring(0, 6);
            return colors[str];
        }
    }
...
//            console.log(response);
    ...
    function onError(response) {
        if (trendsSupported && activeScale==1 && (response === undefined || response.status==500)) {
            d3.select(arg.bindTo)
                .append("div")
                .attr('class', 'warning')
                .html("The zabbix api doesn't support 'trend.get()'.<br>"+
                      "Upgrade to Zabbix 3 or see "+
                      "<a href=\"https://support.zabbix.com/browse/ZBXNEXT-1193\">https://support.zabbix.com/browse/ZBXNEXT-1193</a>");
    ...
    function plotBands(clss, timeInterval, intervals, y, h) {
        function labelDay(d) {
            var a = new Date(d[0]);
            var name = isHoliday(a);
            if (!name) name = days[a.getDay()];
            var pixels = x(d[1]) - x(d[0]);
            if (pixels > 160) return name + ', ' + a.getDate() + '.' + (a.getMonth()+1) + '.' + (a.getFullYear());
            if (pixels > 120) return name + ', ' + a.getDate() + '.' + (a.getMonth()+1) + '.';
            if (pixels > 80) return name;
            if (pixels > 20) return name.substr(0,2);
            if (pixels > 10) return name.substr(0,1);
            else return '';
        }
        var bandClass = {
            'm': function(d,i) {
        ...
        var bandsEnter = timeBands.enter()
            ...
            .attr('width', function(d) { return Math.round(x(d[1])-x(d[0])); })
            .attr('height', h)
            .attr('class', bandClass[timeInterval]);
    ...
    function startFollowView() {
        followViewSeconds = Math.floor((xRange[1]-xRange[0])/width);
        setTimeout(followView, followViewSeconds);
    }

    function followView() {
        if (!followViewSeconds) return;
        var length = xRange[1] - xRange[0];
        xRange[1] = new Date();
        xRange[0] = xRange[1] - length;
        plotSVG();
        setTimeout(followView, followViewSeconds);
    }
    ...
    function plotFuture(t) {
        ...
        if (x(now) <= width + margin.left) {
            ...
            if (now - lastRequest > 10000) startRequests();
    ...
    function plotLegend() {
        ...
        var text = [];
        //TODO: items[index].name + items[index].key
        for (var index in items) {
            text.push(items[index].name);
            console.log(items[index].name + ':' + items[index].key);
        }
    ...
    function plotInfo() {
        ...
        var infoText = svg.select(".info").selectAll("text")
            .data(selected, function(s,i) { return i; })
            .attr("x", mx+14)
            .attr("y", function(s,i) { return my+12 + 12*i; })
            .text(textGenerator);
        infoText.enter()
            .insert("text")
            .attr("x", mx+14)
