...
function triggerTable(arg) {
    ...
    function row(d) {
        ...
        var url = d.url; // url not supported in discovery
        if (!url) {      // so let's take the url from comments' field
            var pattern = /http.*/g;
            url = pattern.exec(d.comments);
        }
        // TODO: do this in a cleaner way
        return '<table class=triggers><tr><td class=description><b>' +
            d.hosts[0].name + '</b>: ' +
            ((url)?'<a href="'+url+'" target=\"_blank\">':'') + d.description +
            ((url)?'</a>':'') +
