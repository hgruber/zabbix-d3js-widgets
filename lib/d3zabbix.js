...
function timeSeries(arg) {
    ...
    var useTrends = typeof arg.useTrends !== 'undefined' ? arg.useTrends : false;
    var trends_scale = 0.3;
    scales.forEach(function(o,i) {
        values[i] = [];
    });
    var activeScale = 0;
    ...
    valueline[1] = d3.svg.line()
        .x(function(d) { return Math.round(x(d[0]*1000)); })
        .y(function(d) { return Math.round(Number(y(d[1]))); })
        .interpolate("cardinal"); // see http://www.d3noob.org/2013/01/smoothing-out-lines-in-d3js.html
    var path = [];
    var path_minmax = [];
    ...
    var yLabel = svg.append("text")
            ...
            .attr("transform", "translate(12,"+margin.top+") rotate(-90)")
        ...
        req('item.get', {
            ...
            if (r.result.length>0) {
                r.result.forEach(function(o, i) {
                    items[o.itemid] = { 'index': i, 'name': o.name, 'key': o.key_, 'units': o.units, 'value_type': o.value_type };
                    scales.forEach(function(o,j) {
                        values[j][i] = [];
                    });
                    path_minmax[i] = svg.append("path")
                        .style('stroke', 'none')
                        .style('fill', colors[i])
                        .style('opacity', 0.5);
   ...
   function startRequests() {
        ...
        substraction( [ [xRange[0], xRange[1]] ], range[activeScale]).forEach(function(r, i) {
            req(scales[activeScale]+'.get', {
                "output": 'extend',
    ...
    function onData(response, status) {
        ...
        var received_scale;
        if (response.result.length > 0 && response.result[0].value !== undefined) {
            receivedScale = 0;
            response.result.forEach(function(o) {
                var i = items[o.itemid].index;
                values[receivedScale][i].push([Number(o.clock), Number(o.value)]);
            });
        } else if (response.result.length > 0 && response.result[0].value_avg !== undefined) {
            receivedScale = 1;
            response.result.forEach(function(o) {
                var i = items[o.itemid].index;
                values[receivedScale][i].push([Number(o.clock), Number(o.value_avg), Number(o.value_min), Number(o.value_max), ]);
            });
        } else return;
        var received = [
        ...
        range[receivedScale] = addition(range[receivedScale], [ received ] );
        itemids.forEach(function(item, i) {
            values[receivedScale][i] = values[receivedScale][i].sort(function(a,b) {  // Fix me: this slows down everything
                return b[0] - a[0];
            });
        });
        // replot all
    ...
    function plotAxis() {
        ...
        values[activeScale].forEach(function(v) {
            for (var i=v.length-1; i>=0; i--) {
                a = v[i][0];
                b = Number(v[i][maxIndex]);
                if (x0 < a && a < x1) {
                    if (b > max) max = b;
                }
            }
    ...
    function plotBands(clss, timeInterval, intervals, y, h) {
        ...
        if (clss == 'daybands') {
            if (intervals.length > 200) intervals=[];
    ...
    function plotTimeBands() {
        ...
        if (timeInterval != 'd' && timeInterval != 'Y') intervals = createBands('d');
    ...
    function plotData() {
        var x0 = Math.round(xRange[0]/1000)
        var x1 = Math.round(xRange[1]/1000)
        // calculate the range with missing data
        var missing = substraction( [ [xRange[0], xRange[1]] ], range[activeScale]);
        // calculate ranges with higher or lower resolution if data is missing
        var lower = [];
        var higher = [];
        if (activeScale == 0) lower = intersection( missing, range[activeScale + 1]);
        if (activeScale == 1) higher = intersection( missing, range[activeScale - 1]);
        if (higher.length > 0) {
            // TODO: generate missing data from data in higher resolution
        }
        var ll = lower.length;
        if (ll > 0) {
//            var substitute = values[activeScale + 1][i].filter(function(o) {
//                for (var j = 0; j < ll; j = j + 1) {
//                    if (o[0] >= lower[j][0] && o[0] <= lower[j][1]) return true;
//                }
//                return false;
//            });
            console.log("I have data in lower resolution: " + lower);
//            activeScale = 1;
        }
        path.forEach(function(p, i) {
            // only use values in current viewport
            visibleValues[i] = values[activeScale][i].filter(function(o) {
                if (o[0] >= x0 && o[0] <= x1) return true;
                else return false;
            });
            p.attr("d", valueline[activeScale](visibleValues[i]));
            if (activeScale > 0) {
                ...
                path_minmax[i].attr("d", valueline[1](min.concat(max.reverse())));
            } else {
                path_minmax[i].attr("d", valueline[0]([]));
            }
        });
    }
    ...
    function plotSVG() {
        ...
        if (values[activeScale][0] !== undefined) {
            plotData();
        }
    }
    ...
    function zoomed() {
        ...
        if (useTrends && zoom_scale < trends_scale) activeScale = 1;
        else activeScale = 0;
        plotSVG();
    }
