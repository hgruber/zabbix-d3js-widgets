...
/*      var dontRequest = addition(scales[activeScale].range, []);
    ...
    function onData(response, status) {
        ...
        if (response.result.length > 0 && response.result[0].value !== undefined) {
            ...
            response.result.forEach(function(o) {
                ...
                scales[receivedScale].values[i][Number(o.clock)] = [ Number(o.clock), Number(o.value) ];
            });
            ...
            response.result.forEach(function(o) {
                var i = items[o.itemid].index;
                scales[receivedScale].values[i][Number(o.clock)] = [ Number(o.clock), Number(o.value_avg), Number(o.value_min), Number(o.value_max)];
            });
    ...
    function plotAxis() {
        ...
        scales[activeScale].values.forEach(function(v,i) {
            var result = {}, key;
            visibleValues[i] = [];
            for (key in v) {
                if (x0 < key && key < x1) {
                    a = v[key];
                    visibleValues[i].push(a);
                    if (a[maxIndex] > max) max = a[maxIndex];
                }
            }
            visibleValues[i].sort(function(a,b) {
                return b[0] - a[0];
            });
        });
        yRange = [0,max];
