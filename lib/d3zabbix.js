...
function timeSeries(arg) {
    ...
    svg
        ...
        .on('mouseleave', function() {
            infoCircle.forEach(function(c) {
                c.attr("cx", -100).attr("cy", -100);
            });
        ...
        req('item.get', {
            ...
            if (r.result.length>0) {
                r.result.forEach(function(o, i) {
                    ...
                    infoCircle[i] = info.append("circle")
                        .attr("r", 5)
                        .attr("cx", -100)
                        .attr("cy", -100);
    ...
    function plotInfo() {
        ...
        if (mouseLocksOnNearest) {
            ...
            visibleValues.forEach(function(v, i) {
                ...
                index = index[2];
                selected[i] = [ v[index][0], v[index][0], v[index][1], i ];
            });
        }
        ...
        selected.forEach(function(s,i) { // TODO: use d3 ???
            var text = format(date) + ', ' + s[2];
            infoCircle[i]
                .attr("cx", x(s[1]*1000))
                .attr("cy", y(s[2]))
                .style("fill", colors[s[3]]);
            infoText
                .attr("x", mx+14)
                .attr("y", my+24)
                .style("fill", colors[s[3]])
