...
function timeSeries(arg) {
    ...
    function startRequests() {
        var x0 = Math.round(xRange[0].getTime()/1000),
            x1 = Math.round(xRange[1].getTime()/1000);
    ...
    function plotAxis(t = 0) {
        ...
        var x0 = Math.round(xRange[0].getTime()/1000)
        var x1 = Math.round(xRange[1].getTime()/1000)
    ...
    function dragged(d) {
        var move = x.invert(d3.event.x).getTime() - x.invert(d3.event.x - d3.event.dx).getTime();
        xRange = [
            new Date(xRange[0].getTime() - move),
            new Date(xRange[1].getTime() - move)
        ];
        plotAxis(0);
    }
    function zoomed() {
        ...
        xRange = [
            new Date(mouse_time.getTime() - add * (mx[0] - l_margin) / width ),
            new Date(mouse_time.getTime() + add * (width + l_margin - mx[0]) / width )
        ];
        plotSVG(0);
    }
