        ...
        gant.forEach(function(g, i) {
            ...
            scales[activeScale].values[i].forEach(function(v,j) {
                ...
                if (!gantNames.hasOwnProperty(name)) {
                    if (begin) gantNames[name] = [ v[0], now ];
                    else if (end) gantNames[name] = [ 0, v[0] ];
                    else addGant(g, v[0], v[0], name);
                } else {
                    if (begin) {
                        addGant(g, v[0], gantNames[name][1], name);
                    } else if (end) {
                        addGant(g, gantNames[name][0], v[0], name);
                    }
    ...
    function plotGant() {
        function setGant(selection) {
            selection
                ...
                .attr("y", function(d,i) {
                    return margin.top + gantHeight*d[3];
                })
        ...
        var gantHeight = Math.round((height - margin.top - margin.bottom)/gantLayers);
        var x0 = Math.round(xRange[0]/1000);
        var x1 = Math.round(xRange[1]/1000);
