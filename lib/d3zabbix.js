...
function timeSeries(arg) {
    ...
    var valueline = d3.svg.line()
        .x(function(d) { return x(d.clock*1000); })
    ...
    function onData(response, status) {
        ...
        path.transition()
            .duration(animate)
            .attr("d", valueline(response.result));
        xRange = last(1);
        console.log(xRange);
    }
...
//	    xAxis.orient("bottom").tickFormat(format).tickSize(-height + b_margin + t_margin).ticks(d3.time.hours, 1);
	    xAxis.orient("bottom").tickFormat(format).tickSize(-height + b_margin + t_margin).ticks(d3.time.minutes, 5);
		yAxis.orient("left").tickSize(-width+1);
        xA.transition().duration(animate).call(xAxis)
        yA.call(yAxis)
            .transition().duration(animate);
    }

    function plotFog() {
        fog .transition().duration(animate)
            .attr("x", x(now))
            .attr("y", t_margin)
