...
function timeSeries(arg) {
    ...
    var sizex = $(arg.bindTo).width();
    var sizey = $(arg.bindTo).height();
    var now = new Date();
	var l_margin = 50,
		r_margin = 20,
		t_margin = 10,
	...
	var xRange = today();
	var yRange = [0, 1000];
	var x = d3.time.scale().range([l_margin, width + l_margin]).domain(xRange);
	var y = d3.scale.linear().range([height - b_margin, t_margin]).domain([0, 1000]);
	var xAxis = d3.svg.axis().scale(x).orient("bottom").tickFormat(format).tickSize(-height + b_margin + t_margin).ticks(d3.time.hours, 1),
		yAxis = d3.svg.axis().scale(y).orient("left").tickSize(-width+1);
	...
	var xA = svg.append("g")
		.attr("class", "x axis")
		.attr("transform", "translate(0," + (height - b_margin) + ")").call(xAxis);
	var yA = svg.append("g")
		.attr("class", "y axis")
    ...
    function replot() {
	    ...
	    x = d3.time.scale().range([l_margin, width + l_margin]).domain(xRange);
	    y = d3.scale.linear().range([height - b_margin, t_margin]).domain([0, 1000]);
	    xAxis = d3.svg.axis().scale(x).orient("bottom").tickFormat(format).tickSize(-height + b_margin + t_margin).ticks(d3.time.hours, 1);
		yAxis = d3.svg.axis().scale(y).orient("left").tickSize(-width+1);
        xA.call(xAxis);
        yA.call(yAxis);
        //
