...
function timeSeries(arg) {
    ...
    // Todo: this should be an array containing several ranges, which will be merged automatically on new data.
    var vRange = [Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY];
    var values = [];
    var visibleValues = [];
    var timer = 0;
    // this is the graph: path is the svg element and valueline is its description
    // there will be an array of these to provide more itemids
        ...
        req('item.get', {
            ...
            if (r.result.length>0) {
                r.result.forEach(function(o) {
                    items[o.itemid] = { 'name': o.name, 'key': o.key_, 'value_type': o.value_type };
                });
   ...
   function startRequests() {
    ...
    function onData(response, status) {
        ...
        // throw away all elements from response except for clock and value
        ...
        // value cache is an ordered array of [ t, y ]
    ...
    function plotData() {
        ...
        visibleValues = values.filter(function(o) {
            if (o[0] >= x0 && o[0] <= x1) return true;
            else return false;
        });
        path.attr("d", valueline(visibleValues));
    }
    ...
    function plotInfo() {
        ...
        my = d3.mouse(this)[1];
        mv = +x.invert(mx)/1000;
        var selected = [Number.POSITIVE_INFINITY];
        if (mouseLocksOnNearest) {
            visibleValues.map(function(o) {
                return [ Math.abs(+x(o[0]*1000)-mx) + Math.abs(+y(o[1])-my), o[0], o[1] ];
            }).forEach(function(o) {
                if (o[0] < selected[0]) selected = o;
            });
        } else {
            visibleValues.map(function(o) {
                return [ Math.abs(+x(o[0]*1000)-mx), o[0], o[1] ];
            }).forEach(function(o) {
                if (o[0] < selected[0]) selected = o;
            });
        }
        var date = new Date(Math.round(selected[1]/60)*60000);
        if (selected[0] !== 'undefined') {
            var text = format(date) + ', ' + selected[2];
            infoCircle
                .attr("cx", x(selected[1]*1000))
                .attr("cy", y(selected[2]));
            infoBox
                .attr("x", mx+12)
                .attr("y", my+9)
            ...
            infoText
                ...
                .text(text);
            infoLine
                .attr("x1", x(selected[1]*1000))
                .attr("y1", t_margin)
                .attr("x2", x(selected[1]*1000))
