...
function timeSeries(arg) {
   ...
   function startRequests() {
        ...
        if (zoom_scale < trends_scale) activeScale = 1;
        else activeScale = 0;
    ...
    function onData(response, status) {
        ...
        itemids.forEach(function(item, i) {
            values[receivedScale][i] = values[receivedScale][i].sort(function(a,b) {
                return b[0] - a[0];
            });
        });
        // replot all
    ...
    function plotAxis() {
        ...
        var a, b;
        var maxIndex = 1;
        cacheRate[0] = Math.round(iLength(intersection(range[0],[[xRange[0],xRange[1]]])) * 100 / iLength([[xRange[0],xRange[1]]]));
        cacheRate[1] = Math.round(iLength(intersection(range[1],[[xRange[0],xRange[1]]])) * 100 / iLength([[xRange[0],xRange[1]]]));
        if (zoom_scale < trends_scale) activeScale = 1;
        else activeScale = 0;
    ...
    function plotData() {
        ...
        path.forEach(function(p, i) {
            ...
            // TODO: find left and right values and use array.slice().
    ...
    function plotLegend() {
        var text = [];
        for (var index in items) text.push(items[index].name);
        var legendEntry = legend.selectAll(".legendtext");
        var lE = legendEntry.data(text, function(o,i) {
                return i;
            })
            .attr("x", function() {
                return margin.left + width - 12;
            })
            .attr("y", function(d,i) {
        ...
        lE.exit().remove();
        var bb = legend.node().getBoundingClientRect();
        legendBox
            .attr("x", bb.x - 22)
            .attr("y", bb.y - 22)
    ...
    function plotInfo() {
        ...
        if (selected[0] !== 'undefined') {
            ...
            infoText
                ...
                .text(text);
            var bb = infoText.node().getBoundingClientRect();
            infoBox
                .attr("x", mx+12)
                .attr("y", my+9)
                .attr("width", bb.width)
