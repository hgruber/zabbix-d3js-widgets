...
function timeSeries(arg) {
    ...
    var scales = [
        {   'method': 'history.get',
            ...
            'gants': [],         // Gant chart values (not available in lower resolution)
            ...
            if (r.result.length>0) {
                r.result.forEach(function(o, i) {
                    ...
                    if (data_type[o.value_type] == 'n') {
                        ...
                        gant[i] = plotArea.append("rect")
                        .attr("class", "gant"+i+" normal")
                        .style('stroke', colors[i]);
                    }
...
/*      var dontRequest = addition(scales[activeScale].range, []);
    ...
    function updateVisibleValues(x0, x1) {
        ...
        path.forEach(function(p, i) {
            ...
            // project next points out of view on y-axis
        ...
        gant.forEach(function(g, i) {
            var gantNames = {};
            scales[activeScale].values[i].forEach(function(v,i) {
                ...
                if (!gantNames.hasOwnProperty(name)) {
                    if (begin) gantNames[name] = [ v[0], Number.POSITIVE_INFINITY ];
                    else if (end) gantNames[name] = [ Number.NEGATIVE_INFINITY, v[0] ];
                    else gantItems.push([ v[0], v[0], name ]);
                } else {
                    if (begin) {
                        gantItems.push([ v[0], gantNames[name][1], name ]);
                    } else if (end) {
                        gantItems.push([ gantNames[name][0], v[0], name ]);
                    }
                }
